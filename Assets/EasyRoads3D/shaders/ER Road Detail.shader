...
Shader "EasyRoads3D/ER Road Detail"
{
	Properties
	{
		...
		_DetailNormalMapScale("Normal Map Scale", Range(0 , 4)) = 1
		[Space]
		[Space]
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord22 = IN.ase_texcoord9.zw * float2( 1,1 ) + float2( 0,0 );
				
				float2 texCoord26 = IN.ase_texcoord9.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord26 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord37 = IN.ase_texcoord9.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( tex2D( _DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_DetailNormalMapScale) );
				
				float2 uv_Metallic = IN.ase_texcoord9.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode23 = tex2D( _Metallic, uv_Metallic );
				

				float3 BaseColor = ( ( ( tex2DNode1 * _Color ) * ( tex2D( _DetailAlbedoMap, texCoord22 ) * _Color1 ) ) * float4(2,2,2,2) ).rgb;
				float3 Normal = BlendNormal( unpack5 , unpack36 );
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = ( tex2DNode23.r * _MainMetallicPower );
				float Smoothness = ( tex2DNode23.a * _MainSmoothnessPower );
				float Occlusion = ( tex2DNode23.g * _OcclusionStrength );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 tex2DNode1 = tex2D( _MainTex, uv_MainTex );
				float2 texCoord22 = IN.ase_texcoord4.zw * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 tex2DNode1 = tex2D( _MainTex, uv_MainTex );
				float2 texCoord22 = IN.ase_texcoord2.zw * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
						 ...
						 )
			{
				...
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord26 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord37 = IN.ase_texcoord5.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( tex2D( _DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_DetailNormalMapScale) );
