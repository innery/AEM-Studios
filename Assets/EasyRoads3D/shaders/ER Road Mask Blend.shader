	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord32 = IN.ase_texcoord9.zw * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode30 = tex2D( _Mask, texCoord32 );
				float4 lerpResult16 = lerp( ( tex2DNode1 * _Color ) , ( tex2D( _Albedo, uv_Albedo ) * _Color0 ) , tex2DNode30.g);
				
				float2 texCoord40 = IN.ase_texcoord9.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord40 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord44 = IN.ase_texcoord9.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack23 = UnpackNormalScale( tex2D( _DetailNormalMapScale, texCoord44 ), _BumpScale1 );
				unpack23.z = lerp( 1, unpack23.z, saturate(_BumpScale1) );
				
				float2 uv_Metallic = IN.ase_texcoord9.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode2 = tex2D( _Metallic, uv_Metallic );
				float2 uv_MetallicGlossMap2 = IN.ase_texcoord9.xy * _MetallicGlossMap2_ST.xy + _MetallicGlossMap2_ST.zw;
				...
				float3 Normal = BlendNormal( unpack5 , unpack23 );
				float3 Emission = 0;
				float3 Specular = 0.5;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord32 = IN.ase_texcoord4.zw * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode30 = tex2D( _Mask, texCoord32 );
				float4 lerpResult16 = lerp( ( tex2DNode1 * _Color ) , ( tex2D( _Albedo, uv_Albedo ) * _Color0 ) , tex2DNode30.g);
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord32 = IN.ase_texcoord2.zw * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode30 = tex2D( _Mask, texCoord32 );
				float4 lerpResult16 = lerp( ( tex2DNode1 * _Color ) , ( tex2D( _Albedo, uv_Albedo ) * _Color0 ) , tex2DNode30.g);
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord40 = IN.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord40 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
